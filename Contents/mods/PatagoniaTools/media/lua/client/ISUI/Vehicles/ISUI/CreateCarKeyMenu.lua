...
function ISVehicleMechanics:doPartContextMenu(part, x, y)
    ...
    if part:getId() == "Engine" then
        ...
        local option = self.context:addOption(getText("IGUI_CreateCarKey"), playerObj, ISVehicleMechanics.onCreateCarKeyMenu, part)
        self:doMenuTooltip(part, option, "create_car_key")
    end
end

function ISVehicleMechanics.onCreateCarKeyMenu(playerObj, part)
    ...
    local item = typeToItem["Base.CarKey"][1]
    ...
    if engineCover then
        ...
        ISTimedActionQueue.add(ISCreateCarKey:new(playerObj, part, item, time))
...
function ISVehicleMechanics:doMenuTooltip(part, option, lua, name)
    ...
    if lua == "create_car_key" then
        ...
        local _carKeyInInventory = self.chr:getInventory():getItemFromType("Base.CarKey", true, true)

        if _carKeyInInventory ~= nil then
            if _carKeyInInventory:getName() == getItemNameFromFullType("Base.CarKey") then
                rgb = ISVehicleMechanics.ghs
            else
                rgb = ISVehicleMechanics.bhs
                option.notAvailable = true
            ...
            tooltip.description = tooltip.description .. " " .. rgb .. getItemNameFromFullType("Base.CarKey") .. " <LINE>"
        

        tooltip.description = string.format("%s <RGB:0,1,1> %s <LINE>", tooltip.description, getText("Tooltip_vehicle_CreateCarKeyWarning"))
